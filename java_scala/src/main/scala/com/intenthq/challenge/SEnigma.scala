...
object SEnigma {
  ...
  def decipher(map: Map[Int, Char])(message: List[Int]): String = {
    // this approach fails on the last test as 37 is found before 23
    val keysSorted = map.keys.toList.sorted.reverse
    println(keysSorted)
    keysSorted.foldLeft(message.mkString){(string,key) =>
      map.get(key).map(value => string.replaceAll(key.toString, value.toString)).getOrElse(string)
    }

    /* More correct approach - suffix tree look up - needs debug
